...
# Traccia i grafici
def plot_metrics_separate(ssim_values, psnr_values, pca_components):
    plt.figure(figsize=(12, 8))
    psnr_values = [0 if x is None else x for x in psnr_values]
    #Grafico per SSIM
    plt.subplot(2, 1, 1)
    plt.xticks(pca_components)
    plt.plot(pca_components, ssim_values, marker='o', color='blue', label='SSIM')
    ...
    plt.subplot(2, 1, 2)
    plt.xticks(pca_components)
    plt.plot(pca_components, psnr_values, marker='s', color='orange', label='PSNR')
    ...
    plt.show()
    plt.savefig("ssim_psnr_vs_pca_components.png")
